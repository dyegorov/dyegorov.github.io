...
(() => {
  ...
  // Longest side cap for stored images. Raised to 2600 so full-bleed photos
  // (hero slider, arena) stay sharp on large/retina screens. The cover slots
  // here span the full viewport, so the old 1200px cap looked soft when
  // stretched back up. A 2600px WebP at q=0.9 is ~0.6-1.2MB.
  const MAX_DIM = 2600;
  ...
  const stylesheet =
    ...
    // filled slots drop the gray drop-hint background so contain-fit images
    // don't show letterbox bands around them.
    ':host([data-filled]) .frame{background:transparent}' +
    // .frame img (clipped) and .spill (unclipped ghost + handles) share the
    // same left/top/width/height in frame-%, computed by _applyView(), so the
    // inside-mask crop and the outside-mask spill stay pixel-aligned.
    // touch-action stays default so a touch starting on the image still scrolls
    // the page on mobile; it's locked to none only while reframing (see below).
